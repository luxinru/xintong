...
<script>
	...
	export default {
		...
		mounted() {
			...
			window.returnAlipayCancel = this.returnAlipayCancel;
			window.returnAlipaySuccess = this.returnAlipaySuccess;
			if (this.channel === 'alipay_app') {
					this.alipayApp(this.money);
				
			}else if(this.channel === 'alipay_h5'){
				this.alipayH5(this.money);
			}
			else {
				this.getData();
			}
		...
		methods: {
			returnAlipaySuccess(){
			    window.webkit.messageHandlers.alert.postMessage({'result':'支付成功!'}); 
			},
			 //支付失败或取消了支付 回调
			returnAlipayCancel(){
			    window.webkit.messageHandlers.alert.postMessage({'result':'支付失败或取消了支付!'}); 
			},
			// 请求后端数据
			alipayApp(m = 50) {
				...
				var host = 'https://web.ytgxjspx.com';//'https://' + window.location.host;
				// 获取token
							...
							switch (xhr.readyState) {
								case 4:
									if (xhr.status == 200) {
										...
										window.webkit.messageHandlers.alipay.postMessage({'result':xhr.responseText});
			...
			alipayH5(m = 50) {
				...
				var isAndroid = u.indexOf('Android') > -1 || u.indexOf('Adr') > -1; //android终端
				var isiOS = !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端
				var host = 'https://ls.hzyuechuang.com';//'https://' + window.location.host;
				// 获取token
				var token = localStorage.getItem('token');
				...
				if (/LT-APP/.test(navigator.userAgent)) {
					...
					xhr.open('POST', pay);
					xhr.send('token=' + token + '&type=alipay_h5' + '&money=' + m);
				}else{
					var channel = null;

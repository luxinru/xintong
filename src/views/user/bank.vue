    ...
    <div class="page">
        <template v-if="data.list && data.list.length > 0">
            <div :class="['card-item', select == k ? 'select' : '']" v-for="(v ,k) in data.list" :key="k" @click="selectCard(k)">
                <!-- <img class="bank-bg" src="../../assets/bank/bank_bg.png" /> -->
                <div class="bank-info">
                    <img class="bank-icon" src="../../assets/bank/icon.png" />
                    <span class="bank-name">{{v.bank}}</span>
                    <span class="edit" @click="delBank(v.id)">删除</span>
                </div>
        ...
        <template v-else>
            <img src="../../assets/bank/bank.png" class="none-bank-icon" />
...
<script>
    ...
    export default {
        ...
        methods: {
            delBank(id) {
                Fetch('/user/bank_remove', {id: id}).then((res) => {
                    this.start()
					console.log("11",res)
                });
            },
...
.page{
    ...
    .card-item{
		background: linear-gradient(90deg, #FF3333 0%, #FC7C64 100%);
		border-radius: 9px;
        margin-top: 17px;
        margin-left: 14px;
        width: 347px;
